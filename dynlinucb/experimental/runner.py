...
class StateIndepRunner:
        # create a new runner for a state-independent environment. Add it in the DLB_exp.py

    def __init__(self, environment, agent, n_trials, horizon, action_size, n_actions, actions):
        self.environment = environment
        self.agent = agent
        self.n_trials = n_trials
        self.horizon = horizon
    ...
    def _run_simulation(self):
        ...
        for t in range(self.horizon):
            ...
            if self.action_size > 1:
                if isinstance(action, np.ndarray):
                    reward = self.environment.step(action.reshape(
                        self.action_size, 1), action_vect)
                else:
                    reward = self.environment.step(self.actions[action, :
                                ].reshape(self.action_size, 1), action_vect)
            else:
                reward = self.environment.step(action, action_vect)
            if isinstance(reward, np.ndarray):
                self.agent.update(reward[0, 0])
            else:
                self.agent.update(reward)
